...
d29.opts <- c("State or National \ncampground","Other public \ncampground","A friend's home \nor backyard", "Girl Scout \nCamps")
# convert pair comparison to matrix format
m <- diag(rep(.5, 4))
d <- diag(rep(NA, 4))
tmp <- as.numeric(d29$Mean) - 1 
obs <- as.numeric(d29[,5])

# a simple wald test, two sided 5% significance level
edges <- pnorm(abs(tmp-0.5)*sqrt(obs)) >= (1-0.025) 
...
d29.tab <- as.table(tmp)
colnames(d29.tab) <- d29.opts
row.names(d29.tab) <- d29.opts

d <- d + t(d)

# tmp[i,j]>0.5 indicates i is preferred over j 
nw <- t(tmp>0.5 & d)

nw <- network(nw)


png(filename="M_fig_camp_pref_network.png", width=600)
plot(nw, label=d29.opts, main="Girl Scouts Camping Preference, a Network Illustration")
dev.off()
...
#### Q19 camping miscs
d19 <- read.csv("report_camp_sleep.csv")

opts <- as.character(d19$Question)
opts.item_list <- strsplit(opts, ":")
opts.items <- c() 
for (i in 1:nrow(d19)){
    opts.items <- c(opts.items, opts.item_list[[i]][1], opts.item_list[[i]][2]) 
}
opts.items <- unique(opts.items)
# convert pair comparison to matrix format
m <- diag(rep(.5, length(opts.items)))
d <- diag(rep(NA, length(opts.items)))
rownames(m) <- opts.items
rownames(d) <- opts.items
colnames(m) <- opts.items
colnames(d) <- opts.items
tmp <- as.numeric(d19$Mean) - 1 
obs <- as.numeric(d19[,5])
...
# row id
for (k in 1:nrow(d19)){
    iopt <- opts.item_list[[k]][1]
    jopt <- opts.item_list[[k]][2]
    line <- paste(iopt,':', jopt, sep="")
    lineId <- which(opts == line)
    i <- which(opts.items == iopt)
    j <- which(opts.items == jopt)
    m[i,j] <- tmp[lineId] 
    d[i,j] <- edges[lineId]
}
...
par(opar)
par(cex = 0.5)
png(filename="M_fig_camp_opts_network.png", width=1200)
plot(nw, label=opts.items, boxed.labels=T, main="Girl Scouts Camping Activities Preference, a Network Illustration")
dev.off()

#### Q45, camping sleeping  

d19 <- read.csv("report_camp_45.csv")
...
par(opar)
par(cex = 0.5)
png(filename="M_fig_camp_sleep_network.png", width=1200)
plot(nw, label=opts.items, boxed.labels=T, main="Girl Scouts Camping Sleeping Accomodations Preference, a Network Illustration")
dev.off()
